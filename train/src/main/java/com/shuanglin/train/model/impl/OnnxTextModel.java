...
/**
 * ONNX 文本生成模型实现
 * 架构层级: model:qwen-onnx -> type:textModel -> framework:onnx -> impl:OnnxTextModel
 */
@Slf4j
public class OnnxTextModel extends AbstractOnnxModel implements TextModel {

    public OnnxTextModel(ModelProperties.ModelConfig config) {
        super(config);
    ...
    @Override
    public void load() {
        log.info("加载 ONNX 文本模型: {}, 路径: {}", getModelId(), config.getModelPath());
        // TODO: 使用 DJL ONNX Runtime 加载文本生成模型
        this.loaded = true;
    }

    @Override
    public void unload() {
        log.info("卸载 ONNX 文本模型: {}", getModelId());
        // TODO: 释放模型资源
        this.loaded = false;
    }
